...
## 20. A real CI break: a hard dependency missing from requirements.txt

Pushing Section 17's fix broke CI (`pytest -v` failing on the `backend` job, `frontend` still green), and so did the commit right before it, the one that first added `color_detection.py`. Not caught locally beforehand because the local dev environment already had the missing package installed from earlier, unrelated work, exactly the kind of gap that only surfaces in a genuinely clean environment.

Root cause, confirmed by actually reproducing it in a fresh virtualenv with only `requirements-dev.txt` installed, not guessed from the error message alone: `color_detection.py` does an unconditional `from PIL import Image` at module level, no `try`/`except` fallback, unlike the torch/`open_clip` imports elsewhere in this codebase, which are deliberately optional and degrade gracefully (see `image_similarity.py`, `clip_model.py`). Pillow was only ever listed in `scripts/requirements-eval.txt`, alongside `datasets` and `open_clip_torch`, dependencies for the offline data-pull script, never in `backend/requirements.txt`, the one CI actually installs. That gap was invisible locally because Pillow was already present in every local dev venv from earlier eval-script runs. It became a real, always-hit failure the moment `retriever.py` started importing `color_similarity` from `color_detection.py` at module level for Section 17's fix, since `retriever.py` is imported unconditionally by `app.main`, so every single test's collection failed on a clean install, not just `test_color_detection.py`'s own tests.

Fixed by adding `pillow==12.3.0` directly to `backend/requirements.txt` (matching the version already resolved and working locally) and removing the now-redundant standalone `pillow` line from `scripts/requirements-eval.txt`, which already inherits it via `-r ../requirements.txt`. Verified the fix properly, not just re-pushed and hoped: built a genuinely fresh virtualenv, installed only `requirements-dev.txt` exactly as CI does, and confirmed the previously-reproduced `ModuleNotFoundError` is gone and the full suite passes (77 passed, 6 skipped, the 6 skips being the optional torch/`open_clip` tests this environment intentionally doesn't install, same as CI) before pushing again.

The honest lesson: a dependency used unconditionally by any module reachable from `app.main`'s import chain has to live in `requirements.txt`, not in a script-specific requirements file, no matter how the code got there or how convenient the local venv already having it felt. `scripts/requirements-eval.txt`'s own `-r ../requirements.txt` line exists for exactly this reason, so it's worth remembering it goes one direction: the app's real runtime dependencies flow into the eval scripts' requirements, not the other way around.
